...
int CCfgFileParms::getValueInt(int& val, char* name)
{
	...
	for (i = 0; i < cfgParms.size(); i++) {
		retval = (int)cfgParms[i].name.compare(name);
		if (retval == 0) {
...
void CCfgFileParms::print()
{
	...
	cout << "统一管理平台地址: " << strTmp;
	strTmp = getValueStr("cmdPort");
	cout << "  UDP端口号; " << strTmp << endl;
	strTmp = getValueStr("oneTouchAddr");
	cout << "oneTouch一键启动地址: " << strTmp;
...
//求层次的编号也就是获得层次位置，层次之间以|间隔，没有设备号，会直接用[号开启实体
//返回值1表示最低层，-1表示没找到
...
int CCfgFileParms::getAddrFromSection(string* dev, string* lay, string* ent, string& addr, string& port, int& indexOfDev, int& indexOfThis, int& iLayNo)
{
	...
	for (index = 0; index < archSection.size(); index++) {
		...
		addr = getAddr(&strTmp);
		//通过层次名计算出层次号,1是第一层（本方案已废弃，采用注册+通告的方法获得端口号，所以此处可能无法获得端口号
		iLayNo = getLayerNo(pstrTmp, lay);
		if (iLayNo < 1) {
			//指定的layer名在配置文件中找不到
			return -3;
		}
		//生成UDP端口号,已废弃，改为初始化为空
		port = " ";
...
int CCfgFileParms::read()
{
	...
	while (!cfgFile.eof()) {
		...
		strTmp.erase(std::remove_if(strTmp.begin(), strTmp.end(), isspace), strTmp.end());
